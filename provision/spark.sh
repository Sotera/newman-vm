...
# Spark
if [[ -a /vagrant/artifacts/spark-1.4.1-bin-hadoop2.3.tgz ]]; then
    cp /vagrant/artifacts/spark-1.4.1-bin-hadoop2.3.tgz /tmp/
else
    wget http://www.interior-dsgn.com/apache/spark/spark-1.4.1/spark-1.4.1-bin-hadoop2.3.tgz
fi

tar -zxvf spark-1.4.1-bin-hadoop2.3.tgz

mv spark-1.4.1-bin-hadoop2.3 /usr/lib/

ln -s /usr/lib/spark-1.4.1-bin-hadoop2.3 /usr/lib/spark

cp /vagrant/artifacts/spark-env.sh /usr/lib/spark/conf/

chown vagrant:vagrant -R /usr/lib/spark-1.4.1-bin-hadoop2.3
